// File: scripts/main.js (Versi FINAL dengan SEMUA 12 Modul)

// --- INTI: Jalankan semua modul saat halaman dimuat ---
document.addEventListener('DOMContentLoaded', async () => {
    // 0. Terapkan tema TERSIMPAN secepat mungkin
    applySavedTheme();
...
// --- (FUNGSI BARU) Terapkan Tema Tersimpan (dijalankan di awal) ---
function applySavedTheme() {
    const savedTheme = localStorage.getItem('sitetheme') || 'gradient';
...
function setupLightbox() {
    ...
    galleryItems.forEach(item => {
        item.addEventListener('click', () => { lightbox.classList.add('active'); lightboxImg.src = item.src; });
    });
    if(lightboxClose) {
...
function setupTestimonials() {
    ...
    function showSlide(index) {
        if (!navContainer.children[index]) return; 
        slides.forEach((slide, i) => {
            slide.classList.remove('active');
